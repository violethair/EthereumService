...
const EthereumService = {
    ...
    getBlockByNum(blockNumber, showFullTransaction = true) {
        return new Promise((resolve, reject) => {
            Axios.post(this.NODE, {
                ...
                params: [numberToHex(blockNumber), showFullTransaction],
            })
                .then((res) => {
                    if (res.data.result) {
    ...
    sendCoin(privateKey, to, value, nonce = false) {
        return new Promise((resolve, reject) => {
            ...
            this.sendTransaction(txParams, privateKey)
        ...
        return new Promise((resolve, reject) => {
            ...
            Axios.post(usePreventiveNode ? this.PREVENTIVE_NODE : this.NODE, {
                ...
                .then((res) => {
                    if (!res || !res.data || !res.data.result || res.data.result === '0x') {
                        if (_this.PREVENTIVE_NODE && usePreventiveNode === false) {
                            console.log(
                                `[callSmartContract ${contractAddress} - ${method}(${params.join(',')})] result is ${
                                    res.data.result
                                }. try preventive node...`,
                            );
                            return _this
                ...
                .catch((error) => {
                    if (_this.PREVENTIVE_NODE && usePreventiveNode === false) {
                        console.log(
                            `[callSmartContract ${contractAddress} - ${method}(${params.join(
                                ',',
                            )})] error. try preventive node...`,
                        );
                        console.log(error);
                        return _this
                            .callSmartContract(contractAddress, abi, method, params, enableDecode, true)
    ...
    sendTransaction(txParams, privateKey) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!txParams.nonce)
                    txParams.nonce = await this.getTransactionCount(this.privateKeyToAddress(privateKey));
    ...
    getLogs(fromBlock, toBlock, usePreventiveNode = false) {
        ...
        return new Promise((resolve, reject) => {
            Axios.post(usePreventiveNode ? this.PREVENTIVE_NODE : this.NODE, {
                ...
                id: 1,
                method: 'eth_getLogs',
                params: [{ fromBlock: numberToHex(fromBlock), toBlock: numberToHex(toBlock) }],
            })
                .then((res) => {
                    if (!res || !res.data || !res.data.result) {
                        if (_this.PREVENTIVE_NODE && usePreventiveNode === false) {
                    ...
                    if (res.data.result.length === 0) {
                        if (_this.PREVENTIVE_NODE && usePreventiveNode === false) {
                            console.log(
                                `[getLogs ${fromBlock} -> ${toBlock}] result length is 0. try preventive node...`,
                            );
                            return _this.getLogs(fromBlock, toBlock, true).then(resolve).catch(reject);
                        } else {
    ...
    getLogsByAddress(fromBlock, toBlock, address) {
        return new Promise((resolve, reject) => {
            Axios.post(this.NODE, {
                ...
                id: 1,
                method: 'eth_getLogs',
                params: [{ fromBlock: numberToHex(fromBlock), toBlock: numberToHex(toBlock), address }],
            })
                .then((res) => {
                    if (!res || !res.data || !res.data.result) {
                        return reject('result is null');
